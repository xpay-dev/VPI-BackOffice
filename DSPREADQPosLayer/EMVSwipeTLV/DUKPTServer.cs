...
namespace DSPREADQPosLayer.EMVSwipeTLV
{
    public class DUKPTServer
    {
        ...
        public static String GetDataKeyFromIPEK(String ksn, String ipek)
        {
            byte[] bKSN = DES.String2Hex(ksn);
            byte[] bIPEK = DES.String2Hex(ipek);
            byte[] bKey = GetDataKeyFromIPEK(bKSN, bIPEK);
            String result = DES.Hex2String(bKey);

            return result;
        }

        public static byte[] GetDataKeyFromIPEK(byte[] ksn, byte[] ipek)
        {
            int i;
            byte[] key, keyTemp1, keyTemp2;

            keyTemp1 = GetDataKeyVarFromIPEK(ksn, ipek);
            keyTemp2 = new byte[16];
            for (i = 0; i < 16; i++)
                keyTemp2[i] = keyTemp1[i];
        ...
        public static byte[] GetDataKeyVarFromIPEK(byte[] ksn, byte[] ipek)
        {
            byte[] key;

            key = GetDukptKeyFromIPEK(ksn, ipek);
            key[5] ^= 0xFF;
            key[13] ^= 0xFF;
        ...
        public static byte[] GetDukptKeyFromIPEK(byte[] ksn, byte[] ipek)
        {
            int i, num;
            byte[] key, KSNTemp, counter, counterTemp;
