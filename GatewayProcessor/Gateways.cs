...
namespace GatewayProcessor {
     public class Gateways {
          ...
          #region TrustPay
          public TrustPay.TransactionResponse ProcessTrustPay(TrustPay.Transaction trans, string process) {
          ...
          #region PayMaya P3 Purchase
          public async Task<string> ProcessPayMayaP3Gateway(string request, string secretKey, string referenceNo) {
               return await Task.Run(() => {
                    try {
                         using (var httpClient = new HttpClient { BaseAddress = new Uri(ConfigurationManager.AppSettings["paymayap3"].ToString()) }) {
                              httpClient.DefaultRequestHeaders.TryAddWithoutValidation("Authorization", "Basic " + secretKey);
                              httpClient.DefaultRequestHeaders.TryAddWithoutValidation("Request-Reference-No", referenceNo);
                              ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12 | SecurityProtocolType.Ssl3;
                              using (var content = new StringContent(request, Encoding.UTF8, "application/json")) {
                                   var responseDataObj = string.Empty;
                                   Task.Run(async () => {
                                        using (var responseData = await httpClient.PostAsync("pay", content)) {
