...
namespace SDGBackOffice
{
    public class CustomAttributes
    {
        ...
        public sealed class ValidateLoginOnPageRequestAttribute : FilterAttribute, IAuthorizationFilter
        {
            string redirectUrl = ConfigurationManager.AppSettings["AsynAddress"].ToString();

            public void OnAuthorization(AuthorizationContext filterContext)
            {
                var httpContext = filterContext.Controller.ControllerContext.HttpContext;
                //if (!LoginManager.IsActiveLogin(httpContext.Session))
                if (httpContext.Session["DisplayName"] == null)
                {
                    httpContext.Response.Redirect(redirectUrl);//Redirection here as absolute url to domain name
                }
            }
