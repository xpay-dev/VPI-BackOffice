...
namespace WisepadLayer
{
    ...
    {
        public String DecryptTrackData1(String encTrack, String key)
        {
            string data = "";

            DecryptedData decryptedData = decryptTrack(encTrack, "", "", "", key);


            if (decryptedData.track1 != "")
            {
                data = decryptedData.track1;
            }
            else
            {
                data = "";//decryptedData.track3;
            }

            return data;
        }

        private DecryptedData decryptTrack(string encTracks, string ksn, string bdk, string nameField, string key)
        {
            try
            {
                ...
                return new DecryptedData(cardholderName, track1, track2, track3, encTracks);
            }catch(Exception ex)
            {
        ...
        public String DecryptTrackData2(String encTrack, String key)
        {
            string data = "";

            DecryptedData decryptedData = decryptTrack(encTrack, "", "", "", key);

            if (decryptedData.track2 != "")
            {
                data = decryptedData.track2;
            }
            else
            {
                data = "";//decryptedData.track3;
        ...
        public String DecryptC5EmvData(String encTrack, String key)
        {
            string data = "";

            DecryptedData decryptedData = decryptTrackEMVData(encTrack, key);

            if (decryptedData.tlv != "")
            {
                data = decryptedData.tlv;
            }

            return data;
        }

        public String DecryptTrack2EMVData(String encTrack, String key)
        {
            string data = "";
            ...
            {
                int init = 0;
                int end = decryptedData.track2.IndexOf("F");

                string track2 = decryptedData.track2.Substring(init, end);

                data = ";" + track2.Replace("D", "=") + "?";
            }
        ...
        private DecryptedData decryptTrackEMVData(string encTracks, string key)
        {
            try
            {
                String tracks = TripleDES.decrypt_CBC(encTracks, key);
                String track1 = "";
                String track2 = tracks;
                String track3 = "";
                String newTrack = "";
                String tlv = tracks;

                return new DecryptedData("", track1, track2, track3, tlv);
